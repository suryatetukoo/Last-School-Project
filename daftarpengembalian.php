...
$query = <<<QUERY
         ...
         where dt_peminjaman.id_user = '{$_SESSION['id_user']}' and (dt_peminjaman.id_peminjaman is null or dt_peminjaman.konfirmasi IN ('Terkonfirmasi', 'Dikembalikan', 'Belum Dikembalikan'))
         order by id_peminjaman desc
         QUERY;
$semuapengembalian = $koneksi
    ->query($query)
...
<body>
    ...
    <section class="hero" id="home">
        <main class="content">
            <h1>Jangan Sampai Terlambat Mengembalikan Ya! Nanti Kalau Kena Denda Gimana?😥</h1>
        </main>
    </section>
    <!-- Hero Section End -->
    ...
    <div class="container-fluid" id="library">
        <div class="container my-7">
            <h1>Daftar Pengembalianmu</h1>
            <div class="px-lg-5 mt-4">
                <div class="row row-cols-4">
                    <?php foreach ($semuapengembalian as $b) : ?>
                        <div>
                            <div class="card cardscale" style="width: 16rem;">
                                <img src="pict/<?= $b['cover'] ?>" alt="<?= $b['judul_buku'] ?>">
                                <div class="card-body">
                                    <?php if ($b['konfirmasi'] === 'Dikembalikan'): ?>
                                        <button class="btn btn-outline-dark" disabled>Telah dikembalikan</button>
                                    <?php elseif ($b['status_denda'] === 'Didenda'): ?>
                                        <button class="btn btn-outline-danger" disabled>Denda: <?= $b['denda'] ?></button>
                                    <?php else: ?>
                                        <form action="createpengembalian.php" method="post">
                                            <input type="hidden" name="id_peminjaman" value="<?= $b['id_peminjaman'] ?>">
                                            <button class="btn btn-dark">Kembalikan</button>
                                        </form>
                                    <?php endif; ?>
                                </div>
                            </div>
